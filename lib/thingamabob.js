...
function parseMessageType(inputBuffer, readIndex) {
  var firstByte = inputBuffer.readUInt8(readIndex);
...
function IngaServer(options) {
  ...
  function handleConnect(connection, data) {
    var protocolName = protocolNameParser.parse(data, 2);
  ...
  function handleConnection(connection) {
    ...
    connection.on('data', function(data) {
      clearTimeout(t);
      var messageType = parseMessageType(data, 0);
      switch (messageType) {
        case messageTypes.CONNECT:
          handleConnect(connection, data);
        default:
          connection.end();
      };
    });
